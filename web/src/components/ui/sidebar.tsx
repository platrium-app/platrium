...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  const { setSidebarWidth, sidebarWidth, setIsDragging } = useSidebar()

  const handleMouseDown = (e: React.MouseEvent) => {
    // Prevent dragging from selecting text
    e.preventDefault()
    setIsDragging(true)
    let isDraggingFlag = true

    const onMouseMove = (moveEvent: MouseEvent) => {
      if (!isDraggingFlag) return

      const newWidth = moveEvent.clientX
      const maxW = window.innerWidth * 0.25 // 25vw
      const minW = 256 // 16rem in pixels (approx)
  ...
  const maxW = typeof window !== "undefined" ? window.innerWidth * 0.25 : 0
  const isAtMin = widthNum <= 256
  const isAtMax = widthNum >= maxW - 1 // allow 1px rounding error
  const cursorClass = isAtMin
    ? "cursor-e-resize"
    : isAtMax
      ? "cursor-w-resize"
      : "cursor-col-resize"
