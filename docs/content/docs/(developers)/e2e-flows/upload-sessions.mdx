...
Next, the client sends a batch of up to 500 chunk hashes to `POST /files/uploadsession/{sessionId}/chunks`.
The backend reads the JWT, then queries the `ChunkStore` for those hashes. 
...
After the client has uploaded all missing chunks to Object Storage, they hit `POST /files/uploadsession/{sessionId}/commit` and provide the JWT Passport and the ordered list of `(Hash, Receipt)` pairs.
