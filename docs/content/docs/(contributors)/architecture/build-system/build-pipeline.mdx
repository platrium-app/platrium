...
When you run the build commands (like `nx run sdk:build:android`), the pipeline compiles the Rust code into massive `.so` binaries, automatically generates the Kotlin wrapper code, and drops everything into the `sdk/_ffi/android/` folder, ready to be consumed by the Android app!
