...
flowchart TD
    ...
    Start --> Scan[Hash upto 128 chunks]
...
We process the file in batches of **128 chunks**. This batching is critical to avoid spamming the backend with thousands of individual API requests.
...
If we kept 128 chunks of 4MB in memory while waiting for the network API, we would instantly consume 2GB of RAM. By discarding the bytes after hashing, the hash scan phase never exceeds 4MB of memory!
:::
...
By leveraging this two-pass system, if the server already has 499 of the 128 chunks, we completely skip reading 499 chunks back into RAM and skip 499 HTTP PUTs! 
